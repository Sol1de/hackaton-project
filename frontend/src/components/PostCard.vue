<template>
  <Card class="max-w-2xl w-full mx-auto mt-10 p-6 shadow-md">
    <CardHeader>
      <CardTitle class="text-2xl font-bold">Review - One Battle After Another</CardTitle>
    ...
    <CardContent class="mt-6 space-y-4">
      ...
      <div v-for="(comment, index) in comments" :key="index" class="border p-3 rounded-lg bg-gray-50">
      ...
      <div class="mt-4">
        <Textarea
          ...
          class="mb-3"
        />
        <Button @click="addComment" class="w-full bg-blue-600 text-white">
          Ajouter un commentaire
        </Button>
      </div>
    </CardContent>
...
// Liste des commentaires
// Chaque commentaire est un objet { author: string, text: string }
const comments = ref<{ author: string, text: string }[]>([])

// Nouveau commentaire
const newComment = ref('')

// Simuler temporairement l'auteur (quand le backend sera là, ce sera l'utilisateur connecté)
